...
f02a <- function(x) {
  x %>%
    separate(x, sep = " ", into = c("opp", "me")) %>%
    encode_moves(., me, c("X","Y","Z")) %>%
    encode_moves(., opp, c("A","B","C")) %>%
    mutate(score = NA) %>%
    #should've used lookup matrix instead
    mutate(
      score = case_when(
        opp == me ~ me + 3,
        me == 3 & opp == 2 ~ me + 6,
...
encode_moves <- function(df, var, moveset){
  df %>%
    mutate({{var}} := str_replace_all({{var}}, moveset[[1]], "1")) %>%
    mutate({{var}} := str_replace_all({{var}}, moveset[[2]], "2")) %>%
    mutate({{var}} := str_replace_all({{var}}, moveset[[3]], "3")) %>%
