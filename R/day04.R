...
f04a <- function(x) {
  x %>%
    ...
    separate(b, c("b1", "b2"), sep = "-") %>%
    mutate(across(everything(), as.numeric)) %>%
    rowwise() %>%
    mutate(within = contained_within(a1, a2, b1, b2)) %>%
    ungroup() %>%
    summarize(sum(within))
}
...
f04b <- function(x) {
  x %>%
    ...
    mutate(across(everything(), as.numeric)) %>%
    rowwise() %>%
    mutate(within = overlap_at_all(a1, a2, b1, b2)) %>%
    ungroup() %>%
    summarize(sum(within))
}
...
contained_within <- function(a1, a2, b1, b2) {
  if( a1 <= b1 & a2 >= b2){
    return(TRUE)
  } else if ( a1 >= b1 & a2 <= b2) {
    return(TRUE)
...
overlap_at_all <- function(a1, a2, b1, b2){
  if (a2 < b1 | b2 < a1){
    return(FALSE)
  } else {
    return(TRUE)
