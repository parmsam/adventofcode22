...
f01_helper <- function(x) {
  # take base R approach
  ex_data_01 <- as.numeric(x)
  calorie_records <- c()
  calorie_sum <- 0
  # loop through
  for(i in ex_data_01){
    if( !is.na(i) ){
      calorie_sum <- i + calorie_sum
    } else {
      calorie_records <- c(calorie_records, calorie_sum)
      calorie_sum <- 0
    }
  }
  # should have used a vector approach instead - ah well
  # include last result
  if (tail(calorie_records, 1) != calorie_sum){
    calorie_records <- c(calorie_records, calorie_sum)
  }
  return(calorie_records)
}
# f01_helper(example_data_01())
